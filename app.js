...
const zh={soundOff:'聲音 關',soundOn:'聲音 開',soundBlocked:'點此開啟聲音',loading:'準備旅程中…',enter:'隨星光出發',pause:'暫停',resume:'繼續',loadFailed:'場景素材載入失敗，請重新整理'};
...
const additions={en:{loadFailed:'Scene failed to load. Please reload.',loading:'Preparing your journey…',navContact:'Contact',archiveLink:'Past projects ↗',podcastLink:'Podcast ↗',eventShort:'Precision Health Carnival 2026 ↗',eventCredit:'Oct 17–18 · Hosted by CancerFree Biotech'},ja:{loadFailed:'シーンを読み込めませんでした。再読み込みしてください。',loading:'旅の準備中…',navContact:'連絡',archiveLink:'過去の活動 ↗',podcastLink:'ポッドキャスト ↗',eventShort:'精準健康嘉年華 2026 ↗',eventCredit:'10.17–18 · CancerFree Biotech 主催'}};
...
const stage=$('.stage'),world=$('.clubhouse-world'),welcome=$('#intro-screen'),arrival=$('#arrival'),hud=$('#flight-hud'),bar=$('.scene-bar'),progress=$('#flight-progress'),pauseButton=$('#pause'),flightAudio=$('#flight-audio');
...
async function setSound(on){enabled=on;blocked=false;$('#sound-choice').checked=on;music.pause();flightAudio.pause();if(on&&(state==='flight'||state==='arrived'))await audioPlay(state==='flight'?flightAudio:music);updateSound()}
...
function arrive({focus=false}={}){state='arrived';elapsed=DURATION;paused=false;last=0;stage.dataset.state=state;welcome.inert=true;arrival.inert=false;bar.inert=false;hud.hidden=true;flightAudio.pause();draw(1,DURATION/1000);if(enabled)audioPlay(music);if(focus){const h=$('.hero-copy h1');h.tabIndex=-1;h.focus({preventScroll:true})}}
function start(){if(!ready)return;if(designReview){arrive();return;}stage.scrollIntoView({behavior:'instant',block:'start'});enabled=$('#sound-choice').checked;blocked=false;elapsed=0;paused=false;last=0;renderClock=0;music.pause();music.currentTime=0;flightAudio.pause();flightAudio.currentTime=0;if(reduced.matches){arrive({focus:true});return}state='flight';stage.dataset.state=state;welcome.inert=true;arrival.inert=true;bar.inert=true;hud.hidden=false;pauseButton.textContent=t('pause');draw(0,0);audioPlay(flightAudio);pauseButton.focus({preventScroll:true})}
...
$('#enter').addEventListener('click',start);$('#replay').addEventListener('click',start);$('#skip-welcome').addEventListener('click',()=>{enabled=$('#sound-choice').checked;arrive({focus:true})});$('#skip-flight').addEventListener('click',()=>arrive({focus:true}));pauseButton.addEventListener('click',()=>setPaused(!paused));
...
async function prepareScene(){
 ...
 ready=true;if(state==='arrived'){draw(1,DURATION/1000);return;}
 state='idle';stage.dataset.state=state;$('#enter').disabled=false;$('#enter').dataset.i18n='enter';$('#enter').textContent=t('enter');draw(0,0);
 if(designReview||initialHash||reduced.matches)arrive();
 const qp=new URLSearchParams(location.search);
 if(qp.has('motion-preview')){enabled=false;state='flight';stage.dataset.state=state;welcome.inert=true;hud.hidden=false;elapsed=Math.max(0,Math.min(1,Number(qp.get('motion-preview'))||0))*DURATION;setPaused(true);draw(elapsed/DURATION,elapsed/1000);}
}
prepareScene().catch(()=>{$('#enter').textContent=t('loadFailed');$('#skip-welcome').focus();});
